<main class="content-area">

    <?php include_once __DIR__ . '/../templates/descripcion/descripcion.php'; ?>

    <?php
    // Si existe una variable $incluirFormulario = true, incluimos el formulario
    if (!empty($incluirFormulario)) {
        include_once __DIR__ . '/../templates/formularios/formulario.php';
    }
    ?>

    <?php include_once __DIR__ . '/../templates/resultados/resultado.php'; ?>

    <?php include_once __DIR__ . '/../templates/ver/verCodigoPHP.php'; ?>
    <?php include_once __DIR__ . '/../templates/ver/verCodigoJS.php'; ?>
    <?php include_once __DIR__ . '/../templates/ver/verDiagrama.php'; ?>

</main>